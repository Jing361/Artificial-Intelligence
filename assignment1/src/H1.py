...
class MyReflexRobot(ContinuousRobot):
...
class MyRandomReflex(ContinuousRobot):
  def runRobot(self):
    (bstate, dirt) = self.percepts
    # This implements the transition function.  Order matters!
    if(bstate == 'Bump'):
      self.action = ('TurnLeft',random.random() * 10 + 133)
    elif(dirt == 'Dirty'):
      self.action = ('Suck',None)
    else:
      self.action = ('Forward',None)
...
class MyDiscreteRobot(DiscreteRobot):
...
if __name__ == "__main__":
  ...
  #discreteTest()
  #reflexTest()
  testAllMaps(MyReflexRobot, allRooms, 20, -1)
  testAllMaps(MyRandomReflex, allRooms, 20, -1)
  testAllMaps(MyDiscreteRobot, allRooms, 20, -1)
